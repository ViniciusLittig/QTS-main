...
function deleta(id) {
    ...
    find(`https://localhost/QTS-main/controlepessoa.php`, options)
        .then(response => {
            response.json()
                .then(data => {
...
const validaRetorno = (result) => {
    ...
    if (result == true) {
        ...
        alerta(0, 'Cadastro realizado com sucesso!', 'Sucesso!', 'https://localhost/QTS-main/listapessoa');

    } else {
        $('html, body').animate({ scrollTop: 0 }, 300);
        alerta(1, 'Falha no cadastro!', 'Falha!', 'https://localhost/QTS-main/pessoa');
    }
}
